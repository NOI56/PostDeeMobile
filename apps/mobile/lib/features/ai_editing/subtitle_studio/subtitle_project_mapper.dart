...
List<AiEditTranscriptWordResult> _readValidatedWordsForPreparedSegment(
  ...
  for (final source in overlappingSources) {
    ...
    if (validatedWords == null) {
      // A mixed response violates the new all-segments contract. Do not let
      // one missing field silently re-enable legacy raw transcript timing.
      return const [];
    }
...
List<_MappedWord>? _prepareTranscriptWords(
  ...
  bool rejectFragmentedThai = true,
}) {
  ...
  if (rejectFragmentedThai && _hasFragmentedThaiTiming(prepared, language)) {
    return null;
  }
  return prepared;
}
...
List<SubtitleWord> _mapWordsToCue(
  ...
  for (final word in words) {
    final overlapsCue = word.startMs < cueEndMs && word.endMs > cueStartMs;
    if (!overlapsCue) {
      continue;
    }
...
bool _hasFragmentedThaiTiming(
  ...
  final hasStandaloneThaiMark =
      words.any((word) => _isStandaloneThaiMark(word.text));
  ...
  return thaiTokenCount / words.length >= 0.5 &&
      ...
      (hasStandaloneThaiMark || singleRuneCount / words.length >= 0.75);
}
