...
8. AI Auto Editing With Configured Speech-to-Text
   ...
   - Android FFmpeg rendering supplies the selected bundled Noto Sans Thai, Prompt, or Anuphan font to libass and compacts kept audio ranges alongside silence-cut video, preventing missing burned subtitles or audio that outlives the final video frame. Automatic subtitles default to Noto Sans Thai Bold with outline 1 and shadow 0 so Thai combining marks stay separated from the base glyph.
   ...
   - Subtitle Studio now opens after AI prepare and before rendering. It provides local autosaved drafts, live video/text preview, cue text and timing edits, add/delete/split/merge, undo/redo, Noto Sans Thai/Prompt/Anuphan font selection, size, text colour, outline, shadow, line count, and safe top/middle/bottom placement. Confirmed edits are used by both preview and full export without another metered AI request. Active-word karaoke, ASS events, reusable brand-style presets, and physical-device preview/export parity remain release gates.
   ...
   - Pixel 8 emulator acceptance on 2026-07-26 regenerated the 2:30 / 38 MB Thai clip as an exact 60-second preview using the bundled Noto Sans Thai Bold asset. The final Android MP4 kept video and audio streams, and visual checks of `ซ้ำแล้วซ้ำ`, `อีกเพื่อจะ`, and `มีอาหารที่` showed separated Thai upper marks with no clipping or heavy-shadow collision. Version-1 Anuphan draft styles migrate to version 2; Prompt styles remain unchanged and Anuphan can still be selected again.
