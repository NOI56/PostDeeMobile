...
3. **Account Deletion**: Implemented in code. The Profile screen warns that store subscriptions must be managed separately, iOS/macOS Apple users pass a backend readiness check then reauthenticate and revoke Apple access, and `DELETE /account` requires recent Firebase authentication, disconnects PostPeer integrations, removes queued jobs/R2 uploads/database data, and deletes the Firebase identity last. Late RevenueCat events cannot recreate a missing user, and an account-only verifier supports a lost-response retry only after Firebase confirms the UID is gone. Apple Sign-In remains unavailable on Android/web until server-side token revocation exists there. Production still requires `FIREBASE_AUTH_DELETE_ENABLED=true`, `FIREBASE_SERVICE_ACCOUNT_JSON`, an R2 lifecycle safety rule, and real-device/provider deletion tests before this item is launch-complete.
