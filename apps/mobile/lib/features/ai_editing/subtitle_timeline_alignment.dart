...
/// Keeps the final target-length cut from ending in the middle of a subtitle.
///
/// Only the start of the existing trailing cut can move, so this does not
/// reorder selected moments or change the opening hook. Consecutive cues are
/// kept as one phrase while their gaps stay short and the final boundary still
/// fits the target tolerance.
List<SilenceCutRange> alignTargetTailToSubtitleBoundary({
  ...
  // Prefer the furthest phrase boundary that stays within tolerance, but never
  // bridge a meaningful pause to reach another cue.
  ...
  // Fall back to the crossing cue's start only when keeping the complete phrase
  // would exceed the configured target tolerance.
