...
class _AiEditingScreenState extends State<AiEditingScreen> {
  ...
  Future<_PreparedRecipeRenderResult> _renderPreparedRecipe({
    ...
    final needsLocalRender = subtitleSegments.isNotEmpty ||
        validatedSoundEffects.isNotEmpty ||
    ...
    final renderSignature = jsonEncode({
      ...
      'soundEffects': [
        for (final placement in validatedSoundEffects) placement.toJson(),
      ],
      'speed': speed,
    ...
    final request = BurnSubtitleRequest(
      ...
      contrast: options.contrast ?? 0,
      soundEffects: [
        for (final placement in validatedSoundEffects)
          BurnSoundEffectAsset(
            assetPath: findPostDeeSoundEffect(placement.soundId)!.assetPath,
