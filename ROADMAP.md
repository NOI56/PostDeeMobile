...
8. AI Auto Editing With Groq Whisper
   ...
   - Real Staging logs exposed a Gemini Files upload-start HTTP 400 caused by SDK-style `displayName`; the REST request now uses the documented `file.display_name` so visual planning can run instead of falling back to audio before upload.
