...
void main() {
  ...
  test('local color recipe never impersonates AI sound analysis', () {
    final recipe = buildLocalColorAiEditRecipe(durationSeconds: 45);

    expect(recipe.capabilities['sfx']?.enabled, isFalse);
    expect(recipe.capabilities['sfx']?.state, 'skipped');
    expect(recipe.capabilities.containsKey('manualSfx'), isFalse);
  });
}
