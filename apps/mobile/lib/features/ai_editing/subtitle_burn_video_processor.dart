...
/// private-use glyphs bundled in the reviewed PostDee subtitle fonts.
...
String _formatAssCentiseconds(int totalCentiseconds) {
  ...
  final seconds = (totalCentiseconds % 6000) ~/ 100;
...
List<_ResolvedSubtitleWord> _resolveSubtitleWords(SubtitleSegment segment) {
  ...
  for (final word in segment.words) {
    ...
    if (textStart < 0 ||
        !containsOnlyUntimedSubtitleSeparators(
...
String _assAnimationOverride(
  ...
  if (normalized != 'pop' || !isFirstEvent) return '';

  final availableSeconds = cueDurationSeconds < eventDurationSeconds
      ? cueDurationSeconds
      : eventDurationSeconds;
  if (!availableSeconds.isFinite || availableSeconds <= 0) return '';
...
String _buildAssDialogueText({
  ...
  final activeColor = _assColor(activeWordColor, '#00E5A8');
  final baseColor = _assColor(textColor, '#FFFFFF');
  return '$prefixOverride$before{\\1c$activeColor&}$active'
      '{\\1c$baseColor&}$after';
}
...
String _buildAssSubtitleContent(
  ...
  for (final segment in segments) {
    ...
    if (words.isEmpty) {
      final start = _assCentiseconds(segment.start);
      final end = _assCentiseconds(segment.end);
      if (end <= start) continue;
      final duration = (end - start) / 100;
      ...
      events.add(
        'Dialogue: 0,${_formatAssCentiseconds(start)},'
        '${_formatAssCentiseconds(end)},Default,,0,0,0,,$text',
      );
      continue;
    }
    ...
    for (var index = 0; index < orderedBoundaries.length - 1; index += 1) {
      ...
      if (endCentiseconds <= startCentiseconds) continue;
