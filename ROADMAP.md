...
8. AI Auto Editing With Configured Speech-to-Text
   ...
   - Android FFmpeg rendering supplies the selected bundled Bai Jamjuree, Prompt, or Anuphan font to libass and compacts kept audio ranges alongside silence-cut video, preventing missing burned subtitles or audio that outlives the final video frame. Automatic subtitles default to Bai Jamjuree Bold with outline 0.5 and shadow 0; the setup size presets compensate for its smaller em metrics.
   ...
   - Enlarged review rejected the first Noto Sans Thai attempt because `้` still visually joined the `ํ` in `ซ้ำ`. Pixel 8 acceptance was repeated with Bai Jamjuree Bold on 2026-07-26: the 2:30 / 38 MB source produced an exact 60-second Android MP4 with video and audio, while `ซ้ำแล้ว`, `ซ้ำอีกเพื่อ`, and `มีอาหารที่` retained visible gaps between stacked marks. Version-1 Anuphan and version-2 Noto draft styles migrate to version 3; Prompt remains unchanged.
