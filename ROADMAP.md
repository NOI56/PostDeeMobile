...
| AI auto editing | ElevenLabs Scribe v2 + Gemini 3.5 Flash-Lite + mobile FFmpeg | Pro subtitle transcription, visual/transcript highlight planning, optional verified-silence/repeated-speech cleanup, subtitle burn-in, phone-side review, and video export | Backend route, `/ai-edits/prepare` recipe contract, quota ledger, mobile FFmpeg flow, waveform-verified silence, automatic adjacent-repeat detection, per-occurrence keep/remove review, subtitle-safe cleanup, detected count/time summary, reversible supported capabilities, atomic accepted-source state, and Post/manual-editor exits exist. Transcript gaps stay non-executable until the phone confirms the source waveform; probe failure keeps audio and can retry locally without another prepare or charge. Colour-only edits that retain the original duration now use a Pro-gated, unmetered local render without audio extraction, upload, or prepare; shortened or audio-dependent edits retain the API path, and unknown enabled capabilities fail closed. Mobile accepts sources up to 10 minutes, extracts bounded temporary M4A chunks and a whole-duration visual proxy when analysis is needed, and offers a 5-second-to-3-minute shortening slider whose right edge keeps the original. ElevenLabs produces timing; Gemini 3.5 selects one coherent story window with structured JSON and provider-default sampling (no explicit `temperature`); deterministic PostDee rules are the final fallback. Groq is no longer selectable in runtime configuration. Production beat sync and the 3-second hook remain locked behind default-off compile-time flags. | The verified Tasks 3–8 implementation is committed locally in `43fa6e0`; Task 9 Steps 1–5 are complete on deployed SHA `6695e5f1d6050e0656c2bfd591fbbad745d80963`. Pixel 8 `color-local` produced the expected unmetered `_edited.mp4` full export and passed device/render checks, but direct zero-upload/prepare log evidence remains pending. A new 30-day, Speech-to-Text-only Staging key was deployed on the same SHA; the 21:57 ICT `target-30` rerun still failed closed with upstream HTTP `401`, no output, and no PostDee quota change. ElevenLabs showed 9,994/10,000 credits used (6 remaining), making provider-side quota exhaustion the leading diagnosis, not a confirmed response detail. Restore or wait for provider quota under separate approval, then rerun the 38 MB source once before continuing. A separate manual SFX foundation now mixes up to eight bundled procedural sounds locally on original-duration clips while keeping AI `sfx=false`; automated checks pass, but audio preview and physical Android/iPhone listening/A-V-sync evidence remain before release. Record the remaining codec, FPS, file size, audio peak, and A/V sync evidence before claiming renderer acceptance, and re-check provider pricing/docs before Production. |
...
| `docs/superpowers/plans/2026-08-09-ai-sound-effects-foundation-plan.md` | ทำบางส่วน | Manual catalog/model, ten procedural WAV assets with hashes, strict local-only recipe, FFmpeg delay/mix/limiter, setup/review Studio, atomic accepted placements, and preview/export reuse are implemented behind a default-off internal QA gate. AI selection stays disabled with `sfx=false`; the seller-facing manual tool remains hidden until in-app sound preview plus physical Android/iPhone listening, level, and A/V-sync acceptance pass. |
...
8. Harden the Existing AI Auto Editing System
   ...
   - Mobile starts every optional capability off. The seller can explicitly enable subtitles, silence cleanup, or repeated-speech cleanup; with no optional capability selected, target-length shortening can still run by itself. The colour/light and audio-cleanup cards are hidden from setup, with restored values forced off, while legacy internal support remains intact. A disabled `AI ใส่เอฟเฟกต์เสียงให้` card communicates the planned AI direction without calling AI or consuming quota. A separate default-hidden internal QA tool can place up to eight PostDee procedural sounds on an original-duration clip and render them locally without enabling `sfx`; shortened/cut combinations fail closed.
