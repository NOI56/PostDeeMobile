...
| AI auto editing | ElevenLabs Scribe v2 + Gemini 3.5 Flash-Lite + mobile FFmpeg | Pro subtitle transcription, visual/transcript highlight planning, optional silence-candidate analysis/repeated-speech cleanup, subtitle burn-in, phone-side review, and video export | Backend route, `/ai-edits/prepare` recipe contract, quota ledger, mobile FFmpeg flow, internal silence candidates, automatic adjacent-repeat detection, per-occurrence keep/remove review, subtitle-safe cleanup, detected count/time summary, reversible supported capabilities, and Post/manual-editor exits exist. Silence candidates are not executable cuts until the mobile waveform verifier in Tasks 6–7 is complete. Mobile accepts sources up to 10 minutes, extracts bounded temporary M4A chunks and a whole-duration visual proxy, and offers a 5-second-to-3-minute shortening slider whose right edge keeps the original. ElevenLabs produces timing; Gemini 3.5 selects one coherent story window with structured JSON and provider-default sampling (no explicit `temperature`); deterministic PostDee rules are the final fallback. Groq is no longer selectable by runtime configuration. Production beat sync and the 3-second hook remain locked behind default-off compile-time flags. | On the deployed current candidate, reconfirm hidden Staging credentials and repeat the 38 MB real-clip E2E. Confirm only M4A reaches ElevenLabs, only the visual proxy reaches Gemini, temporary media is deleted, fallback rules work when Gemini fails, chosen moments are coherent, repeated-word choices match audio and subtitles, and the original video renders locally. Finish and verify waveform-based silence approval before testing silence removal. Re-check provider pricing/docs before production launch. |
...
8. Harden the Existing AI Auto Editing System
   - The existing ElevenLabs + Gemini + mobile FFmpeg flow already lets Pro users request Thai transcription, receive internal silence candidates, burn in subtitles, review the phone-rendered result, and remove supported AI edits they do not want. Silence is not cut from transcript gaps alone; the mobile waveform verifier remains required before that capability is enabled. Remaining work in this item is release hardening, provider evidence, recovery, and physical-device quality testing.
   ...
   - Android FFmpeg rendering supplies bundled Thai-safe Bai Jamjuree, Prompt, or Anuphan fonts to libass, supports validated active-word ASS with static SRT fallback, and can compact kept audio ranges alongside cut video. The current Task 2 candidate does not feed transcript-derived silence directly into that processor; waveform verification must approve any silence range first.
   ...
   - AI prepare now renders a lightweight result and opens result review first; Subtitle Studio opens only from the explicit review action. It provides local autosaved drafts, active-word live preview with safe static fallback, cue text and timing edits, add/delete/split/merge, undo/redo, Bai Jamjuree/Prompt/Anuphan selection, colours, fade/pop, outline, shadow, and safe top/middle/bottom placement. Confirmed edits re-render locally without another metered AI request. Reusable brand-style presets and physical-device preview/export parity remain release gates.
   ...
   - Silence analysis uses `natural` (1.0 s), `balanced` (0.6 s default), or `compact` (0.4 s) thresholds to report internal transcript-gap candidates only. It excludes leading/trailing gaps and never places a candidate in executable cuts. Tasks 6–7 will intersect candidates with the source waveform before enabling silence removal. Repeated-speech cleanup recommends only safely timed adjacent Thai words/phrases, keeps distributed frequent words, negation, emphasis, numbers/prices, and unsafe fragments, while retaining the old exact filler allowlist only for legacy clients.
   - Mobile fits and aligns AI story-plan cuts before applying supported repetition cleanup. Silence candidates remain inactive until waveform verification is connected, so transcript gaps cannot shorten an export by themselves.
   - Result review reports only edits that are actually available. Silence counts/time remain hidden while candidates are still awaiting waveform verification; repeated-speech summaries do not claim that the exported clip saves exactly the detected duration.
